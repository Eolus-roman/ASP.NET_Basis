...
namespace Smart_House
{
    class Menu
    {
        ...
        public void Show(CreateNew CN)
        {
            ...
            while (true)
            {
                ...
                if (commands.Length == 2)
                {
                    ...
                    if (DevicesDictionary[commands[1]] is IChangeEnum)
                    {
                        IChangeEnum lc = (IChangeEnum)DevicesDictionary[commands[1]];
                        switch (commands[0].ToLower())
                        {
                            case "nextlvl": // Hills, QuickCleanMode, Defrost, Space
                                lc.NextEnum();
                                break;
                            case "previouslvl"://DirtRoad, DailyMode, Default, Riot
                                lc.PreviousEnum();
                                break;
        ...
        public void Help()
        { //Данный список команд не полностью действителен. Т.к. данный проект был переработан для .Net
            Console.WriteLine("\n\tСписок общих команд:");
